...
export default function Archive({ activities = [] }) {
  const completed = activities.filter(a=>a.status==='완료').map(a=>({
    ...
    art:artByType[a.activity_type]||'art-note', members:(a.activity_members||[]).map(m=>m.member_name), note:a.result_note, poster:a.poster_url
  }));
  const all=[...completed,...archives];
...
function ArchiveModal({item,close}){return <div className="modal-backdrop" onClick={close}><div className="modal" onClick={e=>e.stopPropagation()}><button className="modal-close" onClick={close}><X/></button><div className={`modal-visual ${item.art}`}>{item.poster&&<img className="modal-poster" src={item.poster} alt=""/>}<Badge>{item.type}</Badge><h2>{item.title}</h2><p>{item.period}</p></div><div className="modal-body"><h3>프로그램 소개</h3><p>{item.desc}. 구성원들이 함께 문제를 발견하고, 작은 실험을 반복하며 의미 있는 결과를 만들어낸 활동입니다.</p><h3>활동 결과</h3><div className="result-box"><Check/><span>{item.note||'최종 결과물 발표와 회고 완료'}</span></div>{item.members?.length>0&&<><h3>함께한 부원</h3><div className="member-chips">{item.members.map(name=><span className="member-chip" key={name}>{name}</span>)}</div></>}<h3>활동 사진</h3><div className="gallery"><i/><i/><i/></div></div></div></div>}
