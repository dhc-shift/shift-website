...
function EventAdmin({items,refresh,setNotice}){
  const empty={title:'',event_type:'프로젝트',event_date:new Date().toISOString().slice(0,10),end_date:new Date().toISOString().slice(0,10),start_time:'14:00',place:'',description:'',visibility:'public'};
  const [form,setForm]=useState(empty);
  const [editing,setEditing]=useState(null);
  const startEdit=item=>{setEditing(item);setForm({title:item.title,event_type:item.event_type,event_date:item.event_date,end_date:item.end_date||item.event_date,start_time:item.start_time?.slice(0,5)||'14:00',place:item.place,description:item.description,visibility:item.visibility});};
