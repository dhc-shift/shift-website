...
function OrgChart(){return <div className="org-chart"><div className="org-node chief"><span>LEADER</span><small>회장</small><b>유민준</b></div><div className="org-line vertical"/><div className="org-node vice"><span>VICE LEADER</span><small>부회장</small><b>최유진</b></div><div className="org-branches"><div className="org-branch"><div className="org-node division blue"><Lightbulb/><b>학술기획 본부</b><small>프로젝트 기획 · 운영 · 실행</small></div><div className="member-row">{['김대희','김지원'].map(x=><div className="member" key={x}><CircleUserRound/><span>{x}</span></div>)}</div></div><div className="org-branch"><div className="org-node division mint"><HeartHandshake/><b>인사시스템 본부</b><small>인사관리 · 뉴스레터</small></div><div className="member-row">{['김남훈','김시연','이남주'].map(x=><div className="member" key={x}><CircleUserRound/><span>{x}</span></div>)}</div></div></div></div>}
