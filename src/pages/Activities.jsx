...
export default function Activities({ calendarEvents = [], activities = [], user, setPage }) {
  ...
  <section className="section container activities-layout">
    ...
    <div className="recruit-panel"><SectionHead eyebrow="ACTIVITIES" title="모집·진행 중인 활동"/><div className="recruit-list">{visible.length?visible.map(a=><button key={a.id} className={selected?.id===a.id?'selected':''} onClick={()=>setSelectedId(a.id)}><div className={`mini-icon ${typeColor(a.activity_type)}`}><ActivityIcon type={a.activity_type}/></div><div><Badge tone={typeColor(a.activity_type)}>{a.activity_type}</Badge><h3>{a.title}</h3><span>{a.status}{a.capacity?` · ${a.capacity}`:''}</span></div><strong>{a.status==='모집 중'?dday(a.start_date):''}</strong></button>):<div className="board-empty"><CalendarDays/><h3>등록된 활동이 없습니다</h3><p>새 활동이 열리면 이곳에 표시됩니다.</p></div>}</div></div>
    ...
    {selected && <div className="detail-card">{selected.poster_url&&<a className="activity-poster" href={selected.poster_url} target="_blank" rel="noreferrer" title="포스터 크게 보기"><img src={selected.poster_url} alt={`${selected.title} 포스터`}/></a>}<div className="detail-top"><div><Badge tone={typeColor(selected.activity_type)}>{selected.activity_type}</Badge><h2>{selected.title}</h2><p>{selected.description}</p></div><strong>{selected.status==='모집 중'?dday(selected.start_date):selected.status}</strong></div>
